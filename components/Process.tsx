...
export default function Process() {
  ...
  return (
    <section id="method" className="blue-section pt-6 md:pt-8 pb-24 md:pb-28 px-6 text-[#f5f5f5]">
              ...
              <div key={step.title} className="relative md:min-h-[140px]">
                ...
                >
                  <div className="relative h-4 w-4">
                    <span
                      className={`absolute inset-0 rounded-full border border-[#a7c8ff]/60 bg-[#06111f] transition-all duration-500 ${visible ? 'shadow-[0_0_0_6px_rgba(112,165,245,0.16)]' : ''}`}
                    />
                    <span
                      className={`absolute inset-[5px] rounded-full bg-[#dfeeff] transition-opacity duration-500 ${visible ? 'opacity-100' : 'opacity-40'}`}
                    />
                  </div>
                </div>
                ...
                >
                  <div
                    className={`blue-panel-soft relative max-w-[420px] w-full rounded-[18px] p-7 transition-all duration-300 ease-out ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'} hover:-translate-y-1.5 hover:border-white/20 hover:shadow-[0_12px_40px_rgba(0,0,0,0.35)] ${isLeft ? 'md:mr-auto md:text-right' : 'md:ml-auto md:text-left'}`}
                  >
